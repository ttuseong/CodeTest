...
##### 풀이
1. 전달받은 rows와 columns 크기의 행렬을 만들고 순서에 맞게 값을 넣어줍니다.
2. queries 크기만큼 반복하는 반복문을 만든 후 회전해야하는 값들을 위, 오른쪽, 아래, 왼쪽 순으로 저장합니다.
3. 2번에서 저장된 배열에서 가장 작은 값을 answer에 저장합니다.
4. 1번에서 만든 행렬에서 회전되는 위치에 맞게 2번에서 값을 받아 수정합니다.