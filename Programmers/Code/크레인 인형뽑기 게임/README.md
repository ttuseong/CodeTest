...
##### 풀이
- 뽑은 인형을 담을 배열을 만들어 줍니다.
- 이동 경로가 저장된 배열의 길이만큼 반복하는 반복문을 만들어 줍니다.
	- 보드의 높이만큼 반복하는 반복문을 만듭니다.
		-  board의 입력값을 보면 [0,0,0,0,0],[0,0,1,0,3]...으로 진행이 됩니다. 예시에 있는 사진과 비교하면 위아래가 뒤집힌 것을 확인할 수 있습니다. 이 점을 참고하여 예제처럼 해당 라인의 가장 위에 있는 값을 가져옵니다.
		- 가장 위에서부터 0이 아닌 값을 찾습니다.
			- 0이 아닌 경우 이전에 뽑은 인형이랑 같은지 확인합니다.
			-  같은 경우 answer에 2를 더하고, 이전에 뽑은 인형이 저장된 배열에서 가장 마지막 값을 제거합니다. 마지막으로 board에서 이번에 뽑은 인형의 위치에 있는 값을 0으로 변경합니다.
			- 아닌 경우 이전에 뽑은 인형이 저장되는 배열에 이번에 뽑은 인형 값을 저장하고, 값을 0으로 변경합니다.